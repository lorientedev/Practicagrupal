...
var
	i: integer;
BEGIN
	FOR i:= big_number_2.length + 1  TO  big_number_1.length DO
...
BEGIN
		...
		IF big_number_1.length > big_number_2.length THEN
		BEGIN
			greater_number:= big_number_1.length;
			fill( big_number_1, big_number_2);
		end
		ELSE
		begin
			fill( big_number_2, big_number_1);
			greater_number:= big_number_2.length;
		end;
		counter:= 0;
		...
		ELSE
			overflow:= TRUE;
		result_total.length:= NumberOFDigits(result_total) ;
END;
...
BEGIN
  ...
  BEGIN
      ...
      END;
       Check_sign(big_number1,big_number2,result);{ poner el signo de la multiplicacion }
  END;
END;
...
result:T_numbers;
  BEGIN
      ...
      CASE operation OF
        '*': BEGIN
              overflow:=FALSE;
                                      Multiplica (big_number1,big_number2,result,overflow);{meter el codigo de suma anterior }
